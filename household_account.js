...
/*
function check_purpose(callback,purpose){
	var checkSql = "select count(*) as cnt from purpose where purpose = ?";
	con.query(checkSql,[purpose],function(err,result,fields){
...
/*
checkPurpose = function(callback,purpose){
	console.log(purpose);
	var checkSql = "select count(*) as cnt from purpose where purpose = ?";
	con.query(checkSql,[purpose],function(err,result,fields){
...
controller.hears(["(help)","(ヘルプ)"], ['direct_message'], (bot,message) =>{
	bot.reply(message,">支出記録 or 支出登録\n支出金額\n目的\n>金額確認\n目的\n>ユーザ登録\n名前\n>help or ヘルプ");
});
...
controller.hears(["(支出登録)","(支出記録)"], ['direct_message'], (bot,message) =>{
	...
	if(expenditure == undefined || purpose == undefined) bot.reply(message,"2行目:*金額*\n3行目:*目的*");
		...
		if(!isNaN(expenditure)){
			...
			controller.storage.users.get(message.user, function (err, user_info) {
				...
				con.query(confirm_users,[user_info.id],function(err,res,fields){
					...
					// ↓ purposeテーブルに登録されているかチェック
					
					var checkPurpose = "select count(*) as cnt from purpose where purpose = ?";
					con.query(checkPurpose,[purpose],function(err,row){
						if(row[0].cnt == 0) bot.reply(message,"入力した目的がテーブルに登録されてません!");
						else{
		...
		}else bot.reply(message,"2行目には金額を入力!");
	}
});
